...
import transl from '../../data/translations';
...
const errors = {
  ...
  invalidPassword: {
    en:
      'The password you provided is invalid, use 6 or more characters with a mix of numbers and letters (at least one must be uppercase)',
    ru: 'Неверный пароль. Используйте не менее 6 символов, включая цифры и буквы (хотя бы одна должена быть прописной)',
    by:
      'Няправільны пароль, выкарыстоўвайце 6 або больш сімвалаў з сумессю лічбаў і літар (прынамсі адна павінна быць прапісная)',
  },
  registrationError: {
...
const login = async () => {
  ...
  if (email.value === '') {
    showPopover(email, errors.enterEmail[getLanguage()], 'right');
    return false;
  }
  if (password.value === '') {
    showPopover(password, errors.enterPassword[getLanguage()], 'right');
    return false;
  }
  ...
  if (result !== true) {
    showPopover(email, errors.loginError[getLanguage()], 'bottom');
  }
...
const register = async () => {
  ...
  if (password.value !== confirmPassword.value) {
    showPopover(password, errors.passwordMatch[getLanguage()], 'right');
    return false;
  }
  if (!emailValidation(email.value)) {
    showPopover(email, errors.invalidEmail[getLanguage()], 'right');
    return false;
  }
  if (!passwordValidation(password.value)) {
    showPopover(password, errors.invalidPassword[getLanguage()], 'bottom');
    return false;
  }
  const result = await app.register(email.value, password.value, currency.innerText);

  if (result !== true) {
    showPopover(email, errors.registrationError[getLanguage()], 'bottom');
  }
  return result;
};
...
export default function renderAuthorizationPage() {
  ...
  <h3 class="form__header" data-i18n="log">${transl[lang].log}</h3>
  <p class="form__subtitle" data-i18n="tagline">${transl[lang].tagline}</p>
  <div class="form__body">
    <div class="form-group">
      <input type="email" class="form-control" id="log-in-email" placeholder="${transl[lang].email}" required />
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-envelope" viewBox="0 0 16 16">
        <path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1H2zm13 2.383l-4.758 2.855L15 11.114v-5.73zm-.034 6.878L9.271 8.82 8 9.583 6.728 8.82l-5.694 3.44A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.739zM1 11.114l4.758-2.876L1 5.383v5.73z"/>
      </svg>
    </div>
    <div class="form-group">
      <input type="password" class="form-control" id="log-in-password" placeholder="${transl[lang].pass}" required />
  ...
  <h3 class="form__header" data-i18n="create-acc">${transl[lang]['create-acc']}</h3>
  <p class="form__subtitle" data-i18n="resTagline">${transl[lang].resTagline}</p>
  <div class="form__body">
    <div class="form-group">
      <input type="email" class="form-control" id="sign-up-email" placeholder="${transl[lang].email}" required>
    ...
    <div class="form-group">
      <input type="password" class="form-control" id="sign-up-password" placeholder="${transl[lang].pass}" required>
    ...
    <div class="form-group">
      <input type="password" class="form-control" id="sign-up-confirm-password" placeholder="${transl[lang].confPass}" required>
    ...
    <div class="form-group two-col">
      ...
      <span id="currency-list" data-i18n="currency">${transl[lang].currency}</span>
    </div>
    <button class="btn btn-yellow" id="submit" data-auth="sign up" data-i18n="sign up">${transl[lang]['sign up']}</button>
    </div>`;
