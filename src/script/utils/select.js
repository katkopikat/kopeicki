  ...
  const getTemplate = (data) => {
    const options = data.list.map((item) => {
      if (data.placeholder === item) {
        return `<li class="select__item selected" id="${item}">${data.isTranslatable ? translate(item) : item}</li>`;
      }
      return `<li class="select__item" id="${item}">${data.isTranslatable ? translate(item) : item}</li>`;
    });

    return `
    <div class="select__backdrop"></div>
    <div class="select__input ${data.class}">
      <span class="select__value" id="${data.placeholder}">
        ${data.isTranslatable ? translate(data.placeholder) : data.placeholder}
      </span>
